...
And there it is. Click on the cube and it will change its color.
We can also use the `useEffect` hook to set up side-effects, for example an interval that changes the cube color randomly every second.
...
We can also add animations to our elements using the `m-attr-anim`. Let's add a rotation animation to the cylinder.
